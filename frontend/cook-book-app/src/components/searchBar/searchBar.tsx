import React, { HtmlHTMLAttributes, KeyboardEvent, useCallback, useState } from "react";
...
export default function SearchBar(props: SearchBarProps) {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement>) =>{
    ...
    if (props.inputTextChange)
       props.inputTextChange(inputText);
  }
    ...
    <div className="rounded-lg">
      <div className="flex items-center bg-white rounded-lg">
        ...
        <div className="relative w-full rounded-lg">
          ...
          <input type="text" id="simple-search" value={inputText} onKeyDown={handleKeyDown} onChange={ handleChange } className="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-pink-400 focus:border-pink-400 block w-full pl-10 p-2.5  dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500" placeholder="Search" required />
        </div>
        {props.searchButtonHidden ? <></> : <button onClick={onClickSearch} type="button" className="p-2.5 text-sm font-medium text-white bg-gray-500 rounded-lg border border-gray-00 hover:bg-gray-300 focus:ring-2 focus:outline-none focus:ring-pink-400"><svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z"></path></svg></button>}
      </div>
    </div>
  );
}
